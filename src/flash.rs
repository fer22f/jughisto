    ...
    T: Serialize + DeserializeOwned + 'static,
{
    fn respond_to(mut self, req: &HttpRequest) -> HttpResponse {
        ...
        if let Some(msg) = msg {
            if let Ok(json) = serde_json::to_string(&ValuedMessageRef { value: &msg.0 }) {
    ...
    T: Serialize + DeserializeOwned,
{
    ...
    pub fn with_redirect(message: T, location: &str) -> Self {
        let response =
            HttpResponse::SeeOther().append_header((actix_web::http::header::LOCATION, location)).finish();
...
impl<S, B> Transform<S, ServiceRequest> for Flash
where
...
impl<S, B> Service<ServiceRequest> for FlashMiddleware<S>
where
    S: Service<ServiceRequest, Response = ServiceResponse<B>, Error = Error> + 'static,
    B: MessageBody + 'static,
{
